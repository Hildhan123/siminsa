                        ...
                        <div class="row align-items-center no-gutters">
                            ...
                            <div class="col-md-6">
                                <div class="card-body content">
                                    <h5><a href="{{ route('pengumuman.detail', ['pengumuman' => $item->id, 'kelurahan_slug' => $desa->slug, 'slug' => Str::slug($item->judul)]) }}"
                                            class="card-title title text-dark">{{ $item->judul }}</a></h5>
                                    <p class="text-muted mb-0">
                                        {{ strip_tags(Illuminate\Support\Str::words($item->konten, 75)) }}
                                    </p>
                                    <div class="post-meta d-flex justify-content-between mt-3"> <a
                                            href="{{ route('pengumuman.detail', ['pengumuman' => $item->id, 'kelurahan_slug' => $desa->slug, 'slug' => Str::slug($item->judul)]) }}"
