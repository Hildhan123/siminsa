    ...
    <div class="col">
        <div class="card bg-secondary shadow h-100">
            <div class="card-header bg-white border-0">
                <h3 class="mb-0">Edit Tautan</h3>
            </div>
            <div class="card-body">
                <form autocomplete="off" action="{{ route('tautan.update', $tautan->id) }}" method="post" enctype="multipart/form-data">
                    @csrf @method('patch')
                    <div class="form-group">
                        <label class="form-control-label">Gambar</label>
                        <div class="text-center">
                            <img title="Klik untuk ganti gambar" onclick="$(this).siblings('.images').click()" class="mw-100 upload-image" style="max-height: 300px" @if ($tautan->gambar) src="{{$tautan->gambar}}" @else src="{{ asset('storage/upload.jpg') }}" @endif>
                            <input accept="image/*" onchange="uploadImage(this)" type="file" name="gambar" class="images" style="display: none">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="form-control-label">Title</label>
                        <input class="form-control @error('title') is-invalid @enderror" name="title" placeholder="Masukkan title ..." value="{{ $tautan->title }}">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control @error('url') is-invalid @enderror" name="url" placeholder="Masukkan url ex:https://google.com" value="{{ $tautan->url }}">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control @error('target') is-invalid @enderror" name="target" id="target">
                            ...
                            <option value="" {{ $tautan->target == null ? 'selected="true"' : ''  }}>Current Window (default)</option>
                            <option value="_blank" {{ $tautan->target == '_blank' ? 'selected="true"' : ''  }}>New Window (_blank)</option>
                        </select>
                        @error('target') <span class="invalid-feedback font-weight-bold">{{ $message }}</span> @enderror
                    </div>
                    <div class="form-group">
                        <label class="form-control-label">Deskripsi</label>
                        <textarea class="form-control @error('deskripsi') is-invalid @enderror" name="deskripsi">{{ $tautan->deskripsi }}</textarea>
