...
@section('title', $pemerintahan_desa->judul)
    ...
    <div class="container">
        <div class="d-flex justify-content-center">
            <div class="card" style="width: 50em">
                <img  @if ($pemerintahan_desa->gambar) src="{{ $pemerintahan_desa->gambar }}" @else src="{{ asset('storage/noimage.jpg') }}" @endif
                    class="card-img-top">
                <div class="card-body">
                    <h2 class="card-title">{{$pemerintahan_desa->judul}}</h2>
                    <blockquote class="text-muted text-right"> <small>{{ $pemerintahan_desa->created_at }}</small> </blockquote>
                    <p class="card-text">{!! $pemerintahan_desa->konten !!}</p>
                </div>
            </div>
        </div>
    </div>
