...
@section('title', 'Edit Potensi Kelurahan')
                    ...
                    <div class="card shadow h-100">
                        <div class="card-header border-0">
                            <div
                                ...
                                <div class="mb-3">
                                    <h2 class="mb-0">Edit Potensi Kelurahan</h2>
        ...
        <div class="col">
            <div class="card bg-secondary shadow h-100">
                <div class="card-header bg-white border-0">
                    <h3 class="mb-0">Edit Potensi Kelurahan</h3>
                </div>
                <div class="card-body">
                    <form autocomplete="off" action="{{ route('potensi.update', $potensi->id) }}"
                        ...
                        <div class="form-group">
                            ...
                            <div class="text-center">
                                <img title="Klik untuk ganti gambar" onclick="$(this).siblings('.images').click()"
                                    ...
                                    @if ($potensi->gambar) src="{{$potensi->gambar}}" @else src="{{ asset('storage/upload.jpg') }}" @endif>
                                <input accept="image/*" onchange="uploadImage(this)" type="file" name="gambar"
                                    class="images" style="display: none">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="form-control-label">Nama Potensi</label>
                            <input class="form-control @error('nama') is-invalid @enderror" name="nama"
                                placeholder="Masukkan nama ..." value="{{ old('nama', $potensi->nama) }}">
                            @error('nama')
                                <span class="invalid-feedback font-weight-bold">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="form-group">
                            <label class="form-control-label">Konten Potensi</label>
                            <textarea class="form-control @error('konten') is-invalid @enderror" name="konten">{{ old('konten', $potensi->konten) }}</textarea>
                            @error('konten')
                                <span class="invalid-feedback font-weight-bold">{{ $message }}</span>
                            @enderror
                        </div>
